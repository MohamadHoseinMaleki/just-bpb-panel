# secureVpn rename: secureVpn | FLAG Country
param(
...
function Resolve-Geo([string]$ip) {
  ...
  try {
    # IMPORTANT: do not use backtick before ? (breaks URL in PS)
    $geoUrl = 'http://ip-api.com/json/' + $ip + '?fields=status,country,countryCode'
    $resp = Invoke-RestMethod -Uri $geoUrl -TimeoutSec 8
    Start-Sleep -Milliseconds 300
    if ($resp.status -eq 'success') {
      $flag = Get-Flag ([string]$resp.countryCode)
      $country = [string]$resp.country
      $label = if ($flag) { "$flag $country" } else { $country }
      $geoCache[$ip] = $label
      return $label
    }
  } catch {
    Write-Host "  geo fail $ip : $_"
  }
  $geoCache[$ip] = $null
  return $null
}
...
function Get-Name([string]$remark, [int]$index, [string]$line) {
  ...
  # 2) Domain / workers
  if ($hostName -match 'workers\.dev|pages\.dev' -or $r -match 'Domain') {
    return "$ProfileName | Cloudflare"
  }

  # 3) Clean IP domain
  if ($r -match 'Clean\s*IP') {
    if ($hostName) { return "$ProfileName | Clean $hostName" }
...
Write-Host "Configs: $($outLines.Count) | Geo entries: $($geoCache.Count)"
